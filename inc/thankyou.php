...
include('inc/config/db.inc.config.php');
$pid = $_GET['payment_id'];
...
if(mysqli_query($conn,"UPDATE `orders` SET  `seen`=1, `status`='$l' WHERE `order_id`=$oid")){
	...
	<body class="bg-dark">
		<script>
				sweetAlert(
					...
					text: "Just wait for the email!<?php echo $pid;?>",
					type: "success"
					},
					function () {
